...
Shared CSS tokens in `src/index.css`. Use `Button`, `Input`, `Select` / `SelectField`, `DateField`, `TextField`, `NumberInput`, `MoneyInput`, and `Chip` instead of one-off height classes. Amount fields on Update, asset detail, and forms use `MoneyInput` (locale blur, currency unit, a11y) (#236). Assets and Update reorder lists share `SortableRow`. The enter-reorder control is shared `ReorderIconButton` (`icon-xl` ListOrdered) on both pages (#267). While reordering it becomes Save; Assets has no Cancel (#268). `Button` `lg` / `xl` were identical to `default` and were removed (#253). Destructive confirms (delete asset/snapshot, replace backup, wipe) use `ConfirmDialog` / `useConfirm`, not `window.confirm` (#247).
...
Icon-only in dense lists (snapshot pencil, comparison cell) stays `Button` `icon` at 32px so table rows do not grow.
