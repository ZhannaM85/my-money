...
describe('ConversionUnavailableButton (#196)', () => {
  it('turns on FX debug, logs pair and date, and opens the Settings log', async () => {
    ...
    expect(
      await screen.findByRole('heading', { name: 'FX debug' }),
    ).toBeVisible()
    const log = getFxDebugLog()
    expect(
      log.some((entry) => entry.message === 'conversion unavailable'),
    ).toBe(true)
    const details = log.find(
      (entry) => entry.message === 'conversion unavailable',
