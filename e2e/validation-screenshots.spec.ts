...
test('capture Comparison Итого vertically aligned with totals (#263)', async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await seedValidationFixture(page, {
  ...
  await page.goto('/compare')
  await expect(page.getByRole('heading', { name: 'Сравнение' })).toBeVisible()
  const label = page.getByTestId('comparison-total-label')
  await expect(label).toBeVisible()
  await expect(label).toHaveClass(/align-middle/)
  await expect(page.getByTestId('comparison-total-2026-08-17')).toHaveClass(
    /align-middle/,
  )
  await page.screenshot({
    path: join(
      ...
      'validation-proof',
      '263',
      '263-comparison-total-valign.png',
    ),
  })
})
